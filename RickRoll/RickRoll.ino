...
void startBeacons() {
  ...
  // Compatible initialiser.
  wifi_config_t ap_config = {};
  strlcpy((char *)ap_config.ap.ssid, "esp32-beaconspam", 32);
  ap_config.ap.ssid_len = 0;
  strlcpy((char *)ap_config.ap.password, "dummypassword", 64);
...
void setup(void) {
  ...
  M5.Lcd.printf("Running RickRoll SSID...");
  startBeacons();
}
...
void loop() {
  delay(500);
  M5.Lcd.fillTriangle(        random(M5.Lcd.width() - 1), 
                              random(M5.Lcd.height() - 45) + 45, 
